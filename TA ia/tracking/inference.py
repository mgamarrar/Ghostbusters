...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        print(emissionModel)
        """  
        print(busters.getObservationDistribution(2))
        print(self.legalPositions)
        print(pacmanPosition)
        print(noisyDistance)
        trueDistance = util.manhattanDistance((1,4), pacmanPosition)
        print(trueDistance)"""
        "*** YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                 "oldPos: ", oldPos, "oldBelief", self.beliefs[oldPos], " newPos: ", newPos, " prob: ", prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles=[]               
        original=self.legalPositions
        repetitions=self.numParticles/len(self.legalPositions)
        for i in range(repetitions):
            self.particles.extend(original)
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        #Se obtiene la distribucion inicial
        beliefs=self.getBeliefDistribution()
        #Se verifica de que se haya atrapado al fantasma
        if noisyDistance==None :
            #Se coloca todas las particulas en la posicion de la celda
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(self.getJailPosition())
        else:
            #Se actualiza la distribucion de creencias
            flag=False
            for N in beliefs:
                trueDistance = util.manhattanDistance(N, pacmanPosition)
                beliefs[N]=beliefs[N]*emissionModel[trueDistance]
                if beliefs[N]>0: flag=True
            #Se verifica si las particulas recibieron peso de 0    
            if flag: 
                #Se muestrea segun la distribucion
                beliefs.normalize()
                self.particles=[]
                for i in range(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            #Se obtiene una distribucion de creencias para una nueva posicion
            #dada una posicion antigua
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            #Se obtiene la nueva posicion de acuerdo a esta distribucion
            #haciendo uso de un muestreo
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        print("posiciones")
        print([self.legalPositions for i in range(self.numGhosts)])
        print("possiblePositions")
        possiblePositions = [x for x in itertools.product(self.legalPositions,repeat=self.numGhosts)]
